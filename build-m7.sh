...
# build-m7.sh — ASL-M7: Input + Storage + USB Driver PDs
set -e
...
echo "==> ASL-M7: Building five sovereign PD binaries"

# Build Rust staticlib once
cargo build \
...
if [[ "$1" == "build" || "$1" == "all" || "$1" == "" ]]; then
    ...
    $MICROKIT_SDK/bin/microkit \
        asl-microkit/system/asl_m7.system \
        --search-path $BUILD_DIR \
        --board $BOARD \
        --config $CONFIG \
        -o $BUILD_DIR/asl_m7.img \
        -r $BUILD_DIR/asl_m7_report.txt
    echo "    OK: $BUILD_DIR/asl_m7.img"
fi

if [[ "$1" == "run" || "$1" == "all" ]]; then
    echo ""
    echo "==> Booting ASL-M7 on QEMU aarch64..."
    timeout 10 qemu-system-aarch64 \
        -machine virt,virtualization=on,highmem=off \
        -cpu cortex-a53 \
        -m 2G \
        -nographic \
        -device loader,file=${BUILD_DIR}/asl_m7.img,addr=0x70000000,cpu-num=0 \
        2>&1 || true
    echo ""
