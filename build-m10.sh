...
# build-m10.sh — ASL-M10: MCS Scheduler + Core Pinning
set -e
...
echo "==> ASL-M10: MCS Scheduler + Core Pinning — final Track B milestone"

# Step 1: Compile AXON runtime
if [[ "$1" != "run" ]]; then
    echo "--> Compiling AXON runtime..."
    axon build \
        --profile seL4-strict \
        --target aarch64-sel4 \
    ...
    $MICROKIT_SDK/bin/microkit \
        asl-microkit/system/asl_m10.system \
        --search-path $BUILD_DIR \
        --board $BOARD --config $CONFIG \
        -o $BUILD_DIR/asl_m10.img \
        -r $BUILD_DIR/asl_m10_report.txt
    echo "    OK: $BUILD_DIR/asl_m10.img"
fi

if [[ "$1" == "run" || "$1" == "all" ]]; then
    echo ""
    echo "==> Booting ASL-M10 on QEMU aarch64..."
    timeout 10 qemu-system-aarch64 \
        -machine virt,virtualization=on,highmem=off \
        -cpu cortex-a53 -m 2G -nographic \
        -smp 4 \
        -device loader,file=${BUILD_DIR}/asl_m10.img,addr=0x70000000,cpu-num=0 \
        2>&1 || true
    echo ""
    echo "==> ASL-M10 boot complete."
fi
