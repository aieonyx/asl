...
echo "==> ASL-M6: Building sovereign PD binaries (C shim + Rust)"
...
build_pd() {
    ...
    # Step 2: Compile C shim (gcc cross-compiler — no -target flag)
    $CC \
        -nostdlib \
        -ffreestanding \
        -I${SDK_DIR}/include \
        -I${SDK_DIR}/include/sel4 \
        -c asl-microkit/src/${C_SHIM} \
        -o $BUILD_DIR/${PD_NAME}_shim.o

    # Step 3: Link C shim + Rust lib + libmicrokit
    $CC \
        -nostdlib \
...
if [[ "$1" == "build" || "$1" == "all" || "$1" == "" ]]; then
    ...
    $MICROKIT_SDK/bin/microkit \
        asl-microkit/system/asl_m6.system \
        --search-path $BUILD_DIR \
        --board $BOARD \
        --config $CONFIG \
        -o $BUILD_DIR/asl_m6.img \
        -r $BUILD_DIR/asl_m6_report.txt
    echo "    OK: $BUILD_DIR/asl_m6.img"
fi

if [[ "$1" == "run" || "$1" == "all" ]]; then
    echo ""
    echo "==> Booting ASL-M6 on QEMU aarch64..."
    echo "    (timeout 10s)"
    timeout 10 qemu-system-aarch64 \
        ...
        -nographic \
        -serial mon:stdio \
        -device loader,file=${BUILD_DIR}/asl_m6.img,addr=0x70000000,cpu-num=0 \
        2>&1 || true
    echo ""
