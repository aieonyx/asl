...
# build-m13.sh — ASL-M13: Onyxia Browser PD
set -e
...
echo "==> ASL-M13: Building 10 sovereign PDs + Onyxia Browser"
...
if [[ "$1" != "run" ]]; then
    ...
    build_pd "asl-onyxia-pd"      "onyxia_pd.c"
    ...
    $MICROKIT_SDK/bin/microkit \
        asl-microkit/system/asl_m13.system \
        --search-path $BUILD_DIR \
        --board $BOARD --config $CONFIG \
        -o $BUILD_DIR/asl_m13.img \
        -r $BUILD_DIR/asl_m13_report.txt
    echo "    OK: $BUILD_DIR/asl_m13.img"
fi

if [[ "$1" == "run" || "$1" == "all" ]]; then
    echo ""
    echo "==> Booting ASL-M13 on QEMU aarch64..."
    timeout 15 qemu-system-aarch64 \
        -machine virt,virtualization=on,highmem=off \
        -cpu cortex-a53 -m 2G -nographic -smp 4 \
        -device loader,file=${BUILD_DIR}/asl_m13.img,addr=0x70000000,cpu-num=0 \
        2>&1 || true
    echo ""
    echo "==> ASL-M13 boot complete."
fi
