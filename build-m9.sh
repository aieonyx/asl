...
# build-m9.sh — ASL-M9: AXON userspace runtime on seL4
set -e
...
echo "==> ASL-M9: Building 7 sovereign PDs + AXON userspace runtime"

# Step 1: Compile AXON runtime
if [[ "$1" != "run" ]]; then
    echo "--> Compiling AXON userspace runtime..."
    axon build \
        --profile seL4-strict \
        --target aarch64-sel4 \
    ...
    echo "    OK: $BUILD_DIR/asl_runtime_global.o"
    ...
    # AXON-Bridge PD links in the AXON runtime object
    ...
    $MICROKIT_SDK/bin/microkit \
        asl-microkit/system/asl_m9.system \
        --search-path $BUILD_DIR \
        --board $BOARD --config $CONFIG \
        -o $BUILD_DIR/asl_m9.img \
        -r $BUILD_DIR/asl_m9_report.txt
    echo "    OK: $BUILD_DIR/asl_m9.img"
fi

if [[ "$1" == "run" || "$1" == "all" ]]; then
    echo ""
    echo "==> Booting ASL-M9 on QEMU aarch64..."
    timeout 10 qemu-system-aarch64 \
        -machine virt,virtualization=on,highmem=off \
        -cpu cortex-a53 -m 2G -nographic \
        -device loader,file=${BUILD_DIR}/asl_m9.img,addr=0x70000000,cpu-num=0 \
        2>&1 || true
    echo ""
