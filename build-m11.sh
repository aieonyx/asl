...
# build-m11.sh — ASL-M11: HANIEL Engine PD
set -e
...
echo "==> ASL-M11: Building 8 sovereign PDs + HANIEL Engine"

if [[ "$1" != "run" ]]; then
    # Compile AXON runtime
    axon build \
        --profile seL4-strict \
        --target aarch64-sel4 \
    ...
    $MICROKIT_SDK/bin/microkit \
        asl-microkit/system/asl_m11.system \
        --search-path $BUILD_DIR \
        --board $BOARD --config $CONFIG \
        -o $BUILD_DIR/asl_m11.img \
        -r $BUILD_DIR/asl_m11_report.txt
    echo "    OK: $BUILD_DIR/asl_m11.img"
fi

if [[ "$1" == "run" || "$1" == "all" ]]; then
    echo ""
    echo "==> Booting ASL-M11 on QEMU aarch64..."
    timeout 15 qemu-system-aarch64 \
        -machine virt,virtualization=on,highmem=off \
        -cpu cortex-a53 -m 2G -nographic -smp 4 \
        -device loader,file=${BUILD_DIR}/asl_m11.img,addr=0x70000000,cpu-num=0 \
        2>&1 || true
    echo ""
    echo "==> ASL-M11 boot complete."
fi
